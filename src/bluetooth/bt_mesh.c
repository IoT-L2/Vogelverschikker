...
#define OP_NODE_CONFIGURED      ESP_BLE_MESH_MODEL_OP_3(0x01, CID_ESP)
...
static bool     s_is_provisioner   = false;
...
static esp_ble_mesh_model_op_t vendor_ops[] = {
    ...
    ESP_BLE_MESH_MODEL_OP(OP_NODE_CONFIGURED, 0),
...
static void prov_send_pub_set(uint16_t addr)
{
    ...
    esp_ble_mesh_cfg_model_pub_set_t msg = {
        ...
        .publish_ttl        = 7,
        .publish_period     = 0,
        .publish_retransmit = ESP_BLE_MESH_TRANSMIT(0, 10),
        .model_id           = VENDOR_MODEL_ID,
        .company_id         = CID_ESP,
    };
...
static void prov_send_node_configured(void)
{
    ...
    esp_err_t err = esp_ble_mesh_model_publish(
        model, OP_NODE_CONFIGURED, 0, NULL, ROLE_PROVISIONER);
    if (err) {
        ESP_LOGE(TAG, "[prov] node-configured signal failed (err %d)", err);
    } else {
        ESP_LOGI(TAG, "[prov] sent node-configured signal to 0x%04x", s_target_unicast);
    }
}
                            ...
                            esp_ble_mesh_model_cb_param_t *param)
{
    ...
    if (opcode == OP_NODE_CONFIGURED) {
        ESP_LOGI(TAG, "[vendor] node-configured signal received");
...
esp_err_t ble_mesh_upgrade_to_provisioner(void)
{
    ...
    esp_ble_mesh_model_t *vmodel = &vendor_models[0];
    bool subscribed = false;
    for (int i = 0; i < CONFIG_BLE_MESH_MODEL_GROUP_COUNT; i++) {
        if (vmodel->groups[i] == ESP_BLE_MESH_ADDR_UNASSIGNED) {
            vmodel->groups[i] = BROADCAST_GROUP_ADDR;
    ...
    vendor_pub.publish_addr = BROADCAST_GROUP_ADDR;
    vendor_pub.app_idx      = APP_KEY_IDX;
    vendor_pub.ttl          = 7;
...
esp_err_t ble_mesh_broadcast_int(int32_t value)
{
    ...
    uint8_t role = s_is_provisioner ? ROLE_PROVISIONER : ROLE_NODE;

    esp_err_t err = esp_ble_mesh_model_publish(
        model, OP_BROADCAST_SET, sizeof(buf), buf, role);
    if (err) {
